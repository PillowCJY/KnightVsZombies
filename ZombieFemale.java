...
public class ZombieFemale {
	...
	//1 = moving to player, 2 = attacking, 3  = dead
		...
		//zombie position
		private double femaleX ;
		private double femaleY;
		...
		//Constructor 
		public ZombieFemale(double randomX, double randomY, double randomVX, double randomVY){
		...
		public void updateZombie(Knight player, double dt){
			...
			//zombie is dead
				...
				if(femaleX <= player.getKnightX()){ 
					faceRight = true; //zombie should face right
					setFaceLeft(false);
					//move to player
					femaleX+= velocityX * dt;
				}
				else if(femaleX > player.getKnightX() ){
				...
				if(femaleY < player.getKnightY()){
					//zombie is above player
					femaleY += velocityY *dt;
				}
				if(femaleY > player.getKnightY()){
					//zombie below player
					femaleY -= velocityY *dt;
		...
		private boolean playerWithinAttackRange(double playerX, double playerY) {
			...
			if(femaleX >= playerX){
				if(femaleY >= playerY-5.0 && femaleY <= playerY + 25.0){
					if(playerX >= femaleX || playerX +50.0 >=  femaleX){
						return true;
					}
				}
			}
